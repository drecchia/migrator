...
query_source_images() {
  ...
  then
    ...
    then
      ...
      FULL_REPO_LIST=$(curl ${V1_OPTIONS} -sf ${V1_PROTO}://${AUTH_CREDS}@${V1_REGISTRY}/v2/_catalog |  jq -r '.repositories | .[] ' | grep -v "${V1_REPO_IGNORE}" ) || catch_error "curl => API failure getting repo list"
    else
